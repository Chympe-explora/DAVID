...
### 5. Everything is now editable directly in `index.html`
All of the site's real content — every price, every meal and camping item,
the hero headline and description, the guide's name/role/bio, your
Instagram/WhatsApp/UPI/bank details, and every gallery photo's category —
now lives in one place: the `window.KC_CONTENT` block near the top of
`index.html`. Change a value there, save, refresh the page — done. Nothing
else needs to be touched, and you don't need to know any code; just edit the
text between the quotes.

There's also a `qrCode` line in the `KC_IMAGES` block — add a file name
there (e.g. `"images/my-qr.png"`, after uploading that file into `images/`)
to show your real UPI QR code image instead of the placeholder box.

**What this covers:** all wording, prices, and photos a visitor actually
reads — headline, description, prices, meal/gear names, guide bio, contact
and payment details, gallery labels, all images including the QR code.

**What's not covered:** small fixed interface words baked into the app
itself — things like button labels ("Book Now", "Next", "Pay Now"), page
section headings ("Meal Options", "Total Calculator"), and form field
labels ("Full Name", "WhatsApp Number"). These almost never need changing,
but if you'd like any of them reworded too, just tell me which ones and
I'll wire them into `index.html` the same way.
