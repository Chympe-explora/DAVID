...
(function () {
  ...
  // Section on/off switches (edit window.KC_CONTENT.sections in index.html)
  ...
  function sharedTourTotals(f) {
    ...
    return {
      ...
      pricePerPerson: PRICES.sharedTour.perPerson,
      grandTotal: payingPersons * PRICES.sharedTour.perPerson
    };
  }
  ...
  function campingTotals(f) {
    ...
    var tentCost = Number(f.tents || 0) * PRICES.camping.tent;
    var mealsCost = f.meals === "yes" ? payingHeads * PRICES.camping.mealsPerPerson : 0;
    var guideCost = PRICES.camping.overnightGuide;
    ...
    return {
      tentCost: tentCost, mealsCost: mealsCost, guideCost: guideCost,
      foodLines: foodLines, foodCost: foodCost,
      grandTotal: tentCost + mealsCost + guideCost + foodCost
    };
  }
  ...
  function privatePackageTotals(f) {
    ...
    var guideCost = PP.guide; // mandatory, per group
  ...
  function App() {
    ...
    var sharedTourState = useState({ adults: 2, children: 0, childAges: [] });
    var sharedTourForm = sharedTourState[0], setSharedTourForm = sharedTourState[1];

    var campingState = useState({ adults: 2, children: 0, childAges: [], tents: 1, meals: "no", foodQty: {} });
    ...
    var advanceState = useState(500); var advance = advanceState[0], setAdvance = advanceState[1];
    ...
    var packageLabel = pkg === "sharedTour" ? "Shared Tour" : pkg === "camping" ? "Camping" : pkg === "privatePackage" ? (CONTENT.packages.privatePackage && CONTENT.packages.privatePackage.name) || "Private Package" : "";
    ...
    function invoiceLines() {
      ...
      if (pkg === "camping") {
        var lines = [
          ["Camping Tent Cost", money(totals.tentCost)],
          ["Meals Cost", money(totals.mealsCost)],
          ["Overnight Guide Cost", money(totals.guideCost)]
        ];
    ...
    var sharedTourCard = SECTIONS.sharedTourCard && h(
      ...
      h(
        ...
        h("img", { src: CONTENT.sectionImages.trekCard, className: "w-full h-full object-cover group-hover:scale-105 transition duration-700" }),
    ...
    var privatePackageCard = SECTIONS.privatePackageCard && h(
      ...
      h(
        "div", { className: "relative h-[220px] overflow-hidden" },
        h("img", { src: CONTENT.guide.image, className: "w-full h-full object-cover group-hover:scale-105 transition duration-700" }),
    ...
    var sharedTourForm2 = pkg === "sharedTour" && h(
      ...
      h(
        ...
        h("div", null, STB.includedText),
        h("div", { className: "mt-2 text-white/50" }, STB.lunchText),
        h("div", { className: "mt-2 text-white/50" }, STB.batchText)
      ),
      h(
        "div", { className: "grid md:grid-cols-2 gap-5" },
    ...
    var campingForm2 = pkg === "camping" && h(
      ...
      h(
        "div", { className: "grid md:grid-cols-2 gap-5" },
        h("label", { className: "space-y-2 block" }, h("span", { className: "text-xs text-white/60" }, CB.tentsLabel), h("div", { className: "flex items-center justify-between px-4 py-2 rounded-xl bg-white/5 border border-white/10" }, h("span", { className: "text-sm" }, campingForm.tents, " Tent(s)"), h(Stepper, { value: campingForm.tents, min: 1, onChange: function (v) { setCampingForm(Object.assign({}, campingForm, { tents: v })); } }))),
        h(
      ...
      h(
        GlassCard, { className: "p-5 !rounded-[16px] text-xs text-white/50" },
        fill(CB.guideNoteTemplate, { overnightGuide: money(PRICES.camping.overnightGuide), childFreeAge: PRICES.childFreeAge })
      ),
      h(
      ...
      h(
        ...
        h(
              ...
              h(
                ...
                h(
                  ...
                  window.KC_IMAGES.qrCode
                    ? h("img", { src: window.KC_IMAGES.qrCode, className: "w-40 h-40 mx-auto object-contain rounded-[8px]" })
          ...
          h(
            ...
            h(
              "div", null,
              h("label", { className: "text-xs text-white/60" }, t("advancePaymentLabel", "Advance Payment (Min ₹500)")),
              h("input", { type: "number", min: 500, value: advance, onChange: function (e) { setAdvance(Math.max(0, Number(e.target.value))); }, className: "mt-2 w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 outline-none text-sm" }),
              advance < 500 && h("div", { className: "text-[11px] text-red-300 mt-2" }, PAY.advanceHelperText),
              h("div", { className: "mt-2 text-xs text-white/50" }, t("balanceLeftLabel", "Balance left to pay on arrival: ₹"), balanceLeft)
            ),
            h("div", { className: "flex gap-3" }, h("a", { href: whatsappLink(), target: "_blank", className: "flex-1 py-3 rounded-full font-semibold text-center text-sm flex items-center justify-center gap-2 " + (advance >= 500 ? "bg-[#2E8B57] hover:bg-[#257a4b]" : "bg-white/10 opacity-50 pointer-events-none") }, h(Phone, { size: 16 }), PAY.submitWhatsappLabel)),
            h(
              ...
              h("div", { className: "text-[12px] text-white/60" }, fill(PAY.receiptNoteTemplate, { number: CONTENT.whatsappNumber }))
            ),
            h("button", { onClick: function () { setPage(6); }, className: "w-full py-3 rounded-full bg-white text-black font-semibold text-sm" }, t("confirmReceiptButton", "I have sent receipt — Confirm Booking"))
          )
        )
      )
    );
    ...
    var page6 = page === 6 && h(
      ...
      h(
        ...
        h("h2", { className: "mt-6 text-3xl font-bold" }, t("bookingConfirmedTitle", "Booking Confirmed!")),
        ...
        h("button", { onClick: function () { setPage(1); setPkg(null); }, className: "mt-8 w-full bg-[#2E8B57] py-3 rounded-full font-semibold" }, t("backToHome", "Back to Home"))
      )
    );
