...
(function () {
  ...
  function campingTotals(f) {
    ...
    // Flat package price per booking — includes 1 tent, mats, chairs, torch &
    // a bonfire. Not multiplied by tent count. Guide is mandatory but billed
    // as its own separate line below.
    var tentCost = PRICES.camping.tent;
    var mealsCost = f.meals === "yes" ? payingHeads * PRICES.camping.mealsPerPerson : 0;
    var guideCost = PRICES.camping.overnightGuide; // mandatory, charged separately
  ...
  function App() {
    ...
    function invoiceLines() {
      ...
      if (pkg === "camping") {
        var lines = [
          ["Camping Package (Tent, Mats, Chairs, Torch & Bonfire)", money(totals.tentCost)],
    ...
    var sharedTourForm2 = pkg === "sharedTour" && h(
      ...
      h(
        ...
        h("div", { className: "mt-2 text-white/50" }, STB.childFreeText),
      ...
      h(
        ...
        h("div", { className: "text-xs text-white/50" }, STB.lunchIncludes),
    ...
    var campingForm2 = pkg === "camping" && h(
      ...
      h(
        GlassCard, { className: "p-5 !rounded-[16px] text-[13px] text-white/70" },
        h("div", { className: "font-medium text-white mb-2 flex items-center justify-between" }, h("span", null, CB.packageIncludesTitle), h("span", { className: "text-white" }, money(PRICES.camping.tent))),
        h("div", null, CB.packageIncludesText)
      ),
      h(
