...
(function () {
  ...
  function campingTotals(f) {
    ...
    // Overnight guide is mandatory but its cost is already baked into the
    // ₹4,500 package price above — it must NOT be added again on top.
    // Kept as its own field (informational) so the invoice can still show
    // it as an included line item.
    ...
    return {
      ...
      // guideCost intentionally excluded from the total — already included in tentCost
      grandTotal: tentCost + mealsCost + foodCost
    };
  }
    ...
    function invoiceLines() {
      ...
      if (pkg === "camping") {
        var lines = [
          ...
          ["Overnight Guide (mandatory, included in package)", "Included"],
